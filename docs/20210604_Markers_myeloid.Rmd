...
marker_expression 
pheno <- subset(metadata_filt, Stimulation == "unstim" | Stimulation == "ununstim")
exp <- genes_tpm_filt[, rownames(pheno)]
...
pheno <- subset(metadata_filt_ordered, Stimulation == "unstim" | Stimulation == "ununstim")
Stimulation <- metadata_filt_ordered$Stimulation
```

# Myeloid marker expression in cultured versus uncultured samples 
...
select <- order(metadata_comparison$X), decreasing=TRUE))

col_ha = columnAnnotation(Stimulation = c("unstim", "ununstim",
                          col = list(Stimulation = c(unstim="#FF6F00FF", ununstim="#C71000FF")),
                          na_col = "white",
                          border = F)
