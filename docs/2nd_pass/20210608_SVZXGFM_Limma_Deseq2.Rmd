...
### Overlap down SV LPS genes + in vitro up SVZ (logFC > 1) 
...
### Overlap up IFNy SVZ up + in vitro SVZ up (logFC > 1)

```{r overlap_up5, echo=TRUE}
DEG_LPS_SVZ <- read.table("DEG_IFNy_SVZ.txt")
set3 <- subset(DEG_LPS_SVZ, log2FoldChange > 1)
set4 <- subset(DEG_baseline_SVZ, log2FoldChange > 1)
...
### Overlap down IFNy SVZ + in vitro SVZ down (logFC < -1)

```{r overlap_down6, echo=TRUE}
set3 <- subset(DEG_LPS_SVZ, log2FoldChange < -1)
set4 <- subset(DEG_baseline_SVZ, log2FoldChange < -1)
...
### Overlap up IFNy SVZ + in vitro down SVZ (logFC < -1)

```{r overlap_down7, echo=TRUE}
set3 <- subset(DEG_LPS_SVZ, log2FoldChange > 1)
set4 <- subset(DEG_baseline_SVZ, log2FoldChange < -1)
...
### Overlap down SVZ IFNy genes + in vitro up SVZ (logFC > 1) 

```{r overlap_down8, echo=TRUE}
set3 <- subset(DEG_LPS_SVZ, log2FoldChange < -1)
set4 <- subset(DEG_baseline_SVZ, log2FoldChange > 1)
...
### Overlap soqn SVZ ex vivo + in vitro down SVZ (logFC  < -1) 
