...
```{r boxplot_LPS_tyroBP, echo = TRUE}
...
```{r boxplot_CIITA_IFNy, echo = TRUE}
IFNy <- all_res$IFNy
...
p <- ggplot(gene2check_charac, aes(x = Stimulation, y = value, fill = Stimulation)) + 
    ...
    geom_boxplot(fill = NA, outlier.colour = NA, width = 0.25) +
    facet_wrap(~Var1) + theme_classic() 
p + stat_compare_means(label = "p.format", label.x.npc = "centre", method = "wilcox.test")
```
