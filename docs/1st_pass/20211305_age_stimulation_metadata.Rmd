...
# Aging in samples that are stimulated with LPS? Does the immune response change in aging? 
...
# Aging in samples that are stimulated with IFNy? Does the immune response change in aging? 
...
### Scatterplot across aging logFC of LPS stimulated samples divided by baseline expression in 151 genes that are known to be associated with aging and LPS stimulation in 128 LPS stimulated samples (direct-pairwise comparisons)

```{r 270genes_plotlogFC, echo = TRUE}
#remove ununstim samples in metadata
metadata_cultured <- metadata_filt[metadata_filt$Stimulation != "ununstim",]
#check numbers
dim(metadata_cultured)
...
LPS <- genes_counts_cultured[genes_counts_cultured$ensembl %in% LPS2_overlap$ensembl, ]
#select only LPS samples 
metadata_combined <- subset(metadata_cultured, Stimulation == "unstim" | Stimulation == "LPS")
#select only LPS samples in genes counts
LPS2 <-LPS[,metadata_combined$Sample]
#order metadata and genes_counts
LPS_ordered <- LPS2[,rownames(metadata_combined)]

LPS.sub <- metadata_combined[metadata_combined$Stimulation %in% "LPS",]
CTR.sub <- metadata_combined[metadata_combined$Stimulation %in% "unstim",]

LPS.sub$Sample <- gsub('-LPS*','',LPS.sub$Sample)
CTR.sub$Sample <- gsub('-unstim*','',CTR.sub$Sample)

CTR.sub <- CTR.sub[CTR.sub$Sample %in% LPS.sub$Sample,]
LPS.sub <- LPS.sub[LPS.sub$Sample %in% CTR.sub$Sample,]

identical(LPS.sub$Sample,CTR.sub$Sample)

CTR.sub.count <- LPS_ordered[colnames(LPS_ordered) %in% rownames(CTR.sub)]
LPS.sub.count <- LPS_ordered[colnames(LPS_ordered) %in% rownames(LPS.sub)]

FC.sub <- LPS.sub.count / CTR.sub.count
scale(FC.sub)

LPS.mic <- data.frame(cbind(LPS = colMeans(FC.sub), Age = LPS.sub$age))
colnames(LPS.mic) <- c("LPS", "Age")
library(ggpmisc)
formula <- y ~ x
ggplot(LPS.mic, aes_string(y = as.matrix(LPS.mic["LPS"]), x = as.matrix(LPS.mic["Age"]))) + 
    geom_point() + scale_colour_gradient(low = "gray90", high = "black") + stat_smooth(method = lm) + 
    theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(method = lm, 
    linetype = "dashed") + labs(y = "Average expression LPS/unstimulated response genes", x = "Age") + 
    stat_poly_eq(aes(label = c(paste(..adj.rr.label..))), label.x = "right", label.y = 0.37, 
        formula = formula, parse = TRUE, size = 3) + # geom_cor(method='pearson')+
stat_fit_glance(method = "lm", method.args = list(formula = formula), geom = "text", 
    aes(label = paste("P-value = ", signif(..p.value.., digits = 3), sep = "")), 
...
### Scatterplot across aging logFC of IFNy stimulated samples divided by baseline expression in 17 genes that are known to be associated with aging and IFNy stimulation in 79 IFNy stimulated samples (direct pairwise comparisons)

```{r 58genes_plotlogFC, echo = TRUE}
#remove ununstim samples in metadata
metadata_cultured <- metadata_filt[metadata_filt$Stimulation != "ununstim",]
#check numbers
dim(metadata_cultured)
...
IFNy<- genes_counts_cultured[genes_counts_cultured$ensembl %in% IFNy2_overlap$ensembl, ]
#select only IFNy samples 
metadata_combined <- subset(metadata_cultured, Stimulation == "unstim" | Stimulation == "IFNy")
#select only IFNy samples in genes counts
IFNy2 <-IFNy[,metadata_combined$Sample]
#order metadata and genes_counts
IFNy_ordered <- IFNy2[,rownames(metadata_combined)]

IFNy.sub <- metadata_combined[metadata_combined$Stimulation %in% "IFNy",]
CTR.sub <- metadata_combined[metadata_combined$Stimulation %in% "unstim",]

IFNy.sub$Sample <- gsub('-IFNy*','',IFNy.sub$Sample)
CTR.sub$Sample <- gsub('-unstim*','',CTR.sub$Sample)

CTR.sub <- CTR.sub[CTR.sub$Sample %in% IFNy.sub$Sample,]
IFNy.sub <- IFNy.sub[IFNy.sub$Sample %in% CTR.sub$Sample,]

identical(IFNy.sub$Sample,CTR.sub$Sample)

CTR.sub.count <- IFNy_ordered[colnames(IFNy_ordered) %in% rownames(CTR.sub)]
IFNy.sub.count <- IFNy_ordered[colnames(IFNy_ordered) %in% rownames(IFNy.sub)]

FC.sub <- IFNy.sub.count / CTR.sub.count
scale(FC.sub)

IFNy.mic <- data.frame(cbind(IFNy = colMeans(FC.sub)), Age = IFNy.sub$age)
colnames(IFNy.mic) <- c("IFNy", "Age")
library(ggpmisc)
formula <- y ~ x
ggplot(IFNy.mic, aes_string(y = as.matrix(IFNy.mic["IFNy"]), x = as.matrix(IFNy.mic["Age"]))) + 
    geom_point() + scale_colour_gradient(low = "gray90", high = "black") + stat_smooth(method = lm) + 
    theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(method = lm, 
    linetype = "dashed") + labs(y = "Average expression IFNy/unstimulated response genes", x = "Age") + 
    stat_poly_eq(aes(label = c(paste(..adj.rr.label..))), label.x = "right", label.y = 0.37, 
        formula = formula, parse = TRUE, size = 3) + # geom_cor(method='pearson')+
stat_fit_glance(method = "lm", method.args = list(formula = formula), geom = "text", 
    aes(label = paste("P-value = ", signif(..p.value.., digits = 3), sep = "")), 
