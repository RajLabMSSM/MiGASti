...
### number of unique donors before QC

```{r load_file2, echo=FALSE}
metadata_uncultured <- subset(metadata, Stimulation == "ununstim")
#check numbers
length(unique(metadata_uncultured$Donor_id))
save(metadata_uncultured, file = "metadata_uncultured.Rdata")
```
...
# Number of unique donors for all ununstim samples (not cultured/stimulated)
...
#only continue with ununstim samples
metadata_uncultured <- subset(metadata_filt, Stimulation == "ununstim")
#check numbers
length(unique(metadata_uncultured$Donor_id))
save(metadata_uncultured, file = "metadata_uncultured.Rdata")
```
...
metadata_filt <- metadata_uncultured
